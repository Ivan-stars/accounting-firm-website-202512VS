...
def extract_en_block(js_text):
    # Find the start of the 'en' block
    start = js_text.find("'en': {")
    if start == -1:
        print("Could not find 'en': { in lang.js")
        return None
    # Find the matching closing brace for the 'en' block
    brace_count = 0
    in_block = False
    block_lines = []
...
def clean_text(text):
    # Remove any HTML tags (shouldn't be present, but just in case)
...
def extract_visible_text(html):
    ...
    for tb in text_blocks:
        print(tb)
    # Remove duplicates and substrings while preserving order
    seen = set()
    unique_blocks = []
...
required_texts = [
    ...
    "Comprehensive and precise financial services",
...
doc = Document()
doc.add_heading('Raymond C W Tam and Co. Home Content (English)', 0)
...
if missing:
    ...
    doc.save('Home.docx')
    print('Exported Home page content to Home.docx (all required texts found)') 